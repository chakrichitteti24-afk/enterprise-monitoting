    ...
    try {
      ...
      if (nextFail >= 5) {
        ...
        if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
          setErrorMessage('Unable to reach GKCE authentication server. Please check your network connection.');
        } else {
          setErrorMessage(
            msg || `Invalid credentials. (${5 - nextFail} attempt${5 - nextFail === 1 ? '' : 's'} remaining before cooldown)`
          );
        }
      }
    } finally {
